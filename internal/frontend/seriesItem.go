/*
This code was copied and slightly modified from the charmCLI list bubble:
https://github.com/charmbracelet/bubbles/blob/v0.20.0/list/defaultitem.go.
I wanted to match the "stock" appearance as closely as possible while making some
modifications to display more information than the default allows.
*/
package frontend
...
// SeriesDelegate is a standard delegate designed to work in lists. It's
...
// NewSeriesDelegate creates a new delegate with default styles.
...
func (d *SeriesDelegate) SetHeight(i int) {
	d.height = i
}
...
func (d SeriesDelegate) Height() int {
...
func (d *SeriesDelegate) SetSpacing(i int) {
	d.spacing = i
}

// Spacing returns the delegate's spacing.
func (d SeriesDelegate) Spacing() int {
	return d.spacing
}

// Update checks whether the delegate's UpdateFunc is set and calls it.
func (d SeriesDelegate) Update(msg tea.Msg, m *list.Model) tea.Cmd {
	return nil
}

// Render prints an item.
func (d SeriesDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	var (
		title, desc  string
		matchedRunes []int
		s            = &d.Styles
	...
	title = ansi.Truncate(title, textwidth, ellipses)
	if d.ShowDescription {
		var lines []string
		for i, line := range strings.Split(desc, "\n") {
			...
			lines = append(lines, ansi.Truncate(line, textwidth, ellipses))
		}
...
func (d SeriesDelegate) ShortHelp() []key.Binding {
...
func (d SeriesDelegate) FullHelp() [][]key.Binding {
